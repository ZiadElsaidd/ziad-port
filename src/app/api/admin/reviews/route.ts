...
export async function POST(req: NextRequest) {
  try {
    ...
    if (supabase) {
      const toInsert = { author: body.author || 'Anonymous', text: body.text || '', rating: body.rating || 5, created_at: new Date().toISOString() };
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const raw = await readFile(DATA_FILE, 'utf8').catch(() => '[]');
    const items = JSON.parse(raw).filter((r: any) => r.id !== id);
    await writeFile(DATA_FILE, JSON.stringify(items, null, 2), 'utf8');
    return new Response('OK', { status: 200 });
  } catch (err) {
    return new Response(String(err), { status: 500 });
