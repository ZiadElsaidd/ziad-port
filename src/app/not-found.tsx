...
export default function NotFound() {
  ...
  return (
    <main className="relative w-full min-h-screen bg-white overflow-hidden flex flex-col">
      ...
      {/* ── Bottom status bar ─────────────────────────────────────────────── */}
      <motion.div
        className="relative z-10 flex items-center justify-between px-[clamp(1.5rem,5vw,5rem)] pb-8 border-t border-black/5"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <motion.div
        className="absolute right-0 top-0 bottom-0 w-px bg-black/4 hidden lg:block pointer-events-none"
